...
# Version 3.1 - Ohne langdetect für stabiles Railway Deployment
...
app = FastAPI(title="Telegram Anti-Spam Bot Railway", version="3.1.0")
...
def detect_language(text: str) -> str:
    ...
    try:
        ...
        # German indicators
        german_words = ['der', 'die', 'das', 'und', 'ist', 'für', 'mit', 'auf', 'ich', 'du', 'er', 'sie', 'wir', 'aber', 'ein', 'eine', 'haben', 'wird', 'kann', 'dass']
        german_count = sum(1 for word in german_words if f' {word} ' in f' {text_lower} ')
        
        # English indicators  
        english_words = ['the', 'and', 'is', 'for', 'with', 'on', 'you', 'that', 'this', 'have', 'are', 'but', 'can', 'will', 'from', 'they', 'been', 'said', 'what', 'more']
...
🔥 **Top Spam-Keywords:**"""
            ...
            if not top_data.get('top_keywords'):
                ...
                for i, (keyword, count) in enumerate(top_data['top_keywords'][:10], 1):
                    emoji_icon = "🥇" if i == 1 else "🥈" if i == 2 else "🥉" if i == 3 else f"{i}."
                    message += f"\n{emoji_icon} **{keyword}**: {count}x"
